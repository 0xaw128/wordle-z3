...
# defines a word as consisting of n z3 Int char indexes
...
def prompt_input(solver, letter_vars):
    ...
    if gold:
        for character in gold.split("."):
            letter, position = re.findall(r'\D+|\d+', character)
            solver = add_gold_constraint(solver, letter_vars, letter, int(position)) 

    # fully correct letters
    if green:
        for character in green.split("."):
            letter, position, occurrence = re.findall(r'\D+|\d+', character)
